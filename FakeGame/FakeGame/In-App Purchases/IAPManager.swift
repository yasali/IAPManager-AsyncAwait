...
actor IAPManager {
  ...
  func getProducts() async throws -> [SKProduct] {
    guard let productIds = getProductIDs() else {
      throw IAPManagerHelper.IAPManagerError.noProductIDsFound
    }
    
    return try await withCheckedThrowingContinuation { continuation in
      IAPManagerHelper.shared.getProducts(productIDs: productIds) { result in
        switch result {
  ...
  func buy(product: SKProduct) async throws -> Bool {
    guard SKPaymentQueue.canMakePayments() else {
      throw IAPManagerHelper.IAPManagerError.paymentWasCancelled
    }
